...
export const AppProvider: React.FC<AppProviderProps> = ({ children }) => {
      ...
      try {
        ...
        if (token) {
          try {
            ...
            setPuntos(perfil.puntos);
            setDiasSeguidos(perfil.dias_seguidos ?? 0);
            setPalabrasAprendidas(perfil.palabras_aprendidas ?? 0);
  ...
  const handleLogin = useCallback(async (e: React.FormEvent) => {
    ...
    try {
      ...
      setDiasSeguidos(usuario.dias_seguidos ?? 0);
      setPalabrasAprendidas(usuario.palabras_aprendidas ?? 0);
